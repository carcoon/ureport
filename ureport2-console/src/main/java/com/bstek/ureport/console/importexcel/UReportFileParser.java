...
public class UReportFileParser extends ExcelParser  {
	private Map<String,Parser<?>> parsers=new HashMap<String,Parser<?>>();
	public UReportFileParser() {
	...
	public ReportDefinition parse(InputStream inputStream) throws Exception {
		...
		try{
			...
			if(!element.getName().equals("ureport")){
				throw new Exception("Unknow report file.");
			}
			...
			for(Object obj:element.elements()){
				...
				if(parser!=null){
					...
					throw new Exception("Unknow element :"+ele.getName());
				}
			}
	...
	public boolean support(String name) {
		return  name.toLowerCase().endsWith(".ureport.xml");
	}
}
